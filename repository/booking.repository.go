...
func (db *bookingStorage) GetBookingByRoomId(ctx context.Context, roomId int) (model.BookingWithCustomerAndRoom, error) {
	...
	err := db.Sql.Table("bookings").
		...
		room_types.name AS typeroom_name, room_types.description AS typeroom_description, 
		room_types.hourly_price, room_types.daily_price, room_types.max_capacity, room_types.standard_capacity
		`).
